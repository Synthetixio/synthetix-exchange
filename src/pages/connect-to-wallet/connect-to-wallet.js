...
class ConnectToWallet extends Component {
	...
	async componentDidMount() {
		...
		try {
			availableWallets = await synthetixJsTools.signer.getNextAddresses(0, WALLET_PAGE_SIZE);
		} catch (e) {
			console.log('error', e);
		}
	...
	async getSynthBalances(synth) {
		...
		const { formatEther } = synthetixJsTools.synthetixJs.utils;
		if (availableWallets[0] && availableWallets[0].balances && availableWallets[0].balances[synth])
			return;
		const balances = await Promise.all(
			availableWallets.map(wallet => synthetixJsTools.synthetixJs[synth].balanceOf(wallet.address))
		);
	...
	onSelectWallet(index) {
		return async () => {
			...
			const { currentWalletInfo, connectToWallet, changeScreen, toggleLoadingScreen } = this.props;
			const { walletType } = currentWalletInfo;
			if (walletType === 'Metamask' || walletType === 'Coinbase') return;

			const idx = walletSelectorIndex + index;
			const availableWalletsStringArray = availableWallets.map(wallet => wallet.address);
		...
		return (
			<div className={styles.connectToWallet}>
				...
				<div className={styles.connectToWalletHelp}>
					<div className={styles.connectToWalletHelpCaption}>Having issues?</div>
	...
	render() {
		const { walletSelectorIndex, fetchingNextAddresses, availableWallets, connected } = this.state;
...
export default connect(mapStateToProps, mapDispatchToProps)(ConnectToWallet);
