...
class Markets extends Component {
	...
	getPrecision(base, quote) {
		return quote === 'sXAU' && (base === 'sKRW' || base === 'sJPY') ? '0,0.00000000' : '0,0.00000';
	}
	...
	renderTableBody() {
		...
		const filteredSynths = availableSynths.filter(synth => {
			return synth.name !== 'XDR' && synth.name.toUpperCase().includes(inputValue.toUpperCase());
		});
		return filteredSynths.map(baseSynth => {
			...
			return (
				<Fragment key={`bloc-${baseSynth.name}`}>
					...
					{availableSynths
						.filter(quoteSynth => baseSynth.name !== quoteSynth.name && quoteSynth.name !== 'XDR')
						.map(quoteSynth => {
							return (
								<tr
