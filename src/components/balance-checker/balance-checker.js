...
class BalanceChecker extends Component {
	...
	walletHasChanged(prevProps) {
		...
		return currentWalletInfo.selectedWallet !== prevProps.currentWalletInfo.selectedWallet;
	}
	...
	componentDidUpdate(prevProps) {
		const { toggleLoadingScreen } = this.props;
		if (this.transactionWentThrough(prevProps) || this.walletHasChanged(prevProps)) {
			toggleLoadingScreen(true);
			this.refreshData();
		}
	}
	...
	renderWidgetHeader() {
		const { currentWalletInfo } = this.props;
		const buttonIsDisabled = !currentWalletInfo || !currentWalletInfo.selectedWallet;
		return (
				...
				<tbody>
					<tr className={styles.tableBodyRow}>
						...
						<td className={styles.tableBodyBalance}>
							{totalBalance ? `$${numbro(totalBalance).format('0,0.00')} USD` : '--'}
						</td>
					</tr>
					<tr className={styles.tableBodyRow}>
						<td className={styles.tableBodySynth}>ETH</td>
						<td className={styles.tableBodyBalance}>
							<div>{ethBalance ? numbro(ethBalance.amount).format('0,0.00') : '--'}</div>
							<div>{ethBalance ? `$${numbro(ethBalance.value).format('0,0.00')} USD` : null}</div>
						</td>
					</tr>
					<tr>
