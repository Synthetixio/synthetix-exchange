...
class WalletSelectorPopup extends Component {
	...
	registerMetamaskAddressListener = () => {
		...
		if (
			...
			synthetixJsTools.signer.provider._web3Provider.publicConfigStore.on('update', listener);
		}
	};
	...
	onMetamaskAddressChange = async data => {
		...
		if (
			currentWalletInfo.selectedWallet.toLocaleLowerCase() === data.selectedAddress.toLowerCase()
		) {
			return;
		}
		...
		return (
			<Popup isVisible={isVisible} closePopup={this.closePopup}>
				<div>
					...
					<div className={styles.buttonsWrapper}>{WALLET_TYPES.map(this.renderButton)}</div>
				</div>
			</Popup>
		);
	}
