...
import { getSynthToExchange, getSynthToBuy, getExchangeRates } from '../../ducks';
...
class RateList extends Component {
	...
	renderTableBody() {
		const { exchangeRates, synthToExchange, synthToBuy, availableSynths } = this.props;
		...
		return filteredSynths.map((synth, i) => {
			...
			return (
				<tr
					key={i}
					className={synthToBuy && synthToBuy.name === synth.name ? styles.tableRowActive : ''}
