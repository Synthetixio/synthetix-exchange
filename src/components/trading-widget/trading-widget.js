...
class TradingWidget extends Component {
	...
	async confirmTrade() {
		...
		const { selectedWallet, walletType, gasPrice, gasLimit } = currentWalletInfo;
	...
	render() {
		const { synthToBuy, synthToExchange, currentWalletInfo, frozenSynths } = this.props;
		...
		const confirmTradeButtonIsEnabled =
			...
			Number(balances[synthToExchange.name]) > 0;
		const tradeMaxButtonIsEnabled = balances && Number(balances[synthToExchange.name]) > 0;
		return (
			<div className={`${styles.widget} ${!selectedWallet ? styles.widgetIsDisabled : ''}`}>
				...
				{this.renderInputs()}
				<GweiSelector hasGweiLimit={true} />
				<button
				...
				{!selectedWallet ? (
					<Fragment>
						...
						<div className={styles.widgetOverlayMessage}>Please connect your wallet</div>
					</Fragment>
				) : null}
			</div>
		);
...
export default connect(mapStateToProps, mapDispatchToProps)(TradingWidget);
