	...
	render() {
		...
		return (
			<div className={styles.widgetInputWrapper}>
				<OutsideClickHandler onOutsideClick={() => this.setState({ boxIsOpen: false })}>
					<input
						className={`${styles.widgetInputElement} ${
							currentExchangeMode === 'basic' ? styles.widgetInputElementBig : ''
						}`}
						type="text"
						value={value || ''}
						placeholder={0}
					...
					>
						<div className={`${styles.widgetInputInner} ${isFrozen ? styles.isFrozen : ''}`}>
							<img
						...
						{currentExchangeMode === 'basic' ? (
							<div className={styles.widgetInputInner}>
								<img className={styles.inputOpenDropdownIcon} src="/images/angle-down-icon.svg" />
							</div>
						) : null}
					</div>
					{boxIsOpen ? (
...
export default connect(mapStateToProps, mapDispatchToProps)(TradingWidgetInput);
