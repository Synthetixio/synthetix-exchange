...
class DepotPopup extends Component {
	...
	async buyFromDepot() {
		...
		try {
			...
			const transactionRejected = e.message && e.message.includes(TRANSACTION_REJECTED[walletType]);
	...
	async componentDidUpdate(prevProps) {
		if (!prevProps.isVisible && this.props.isVisible) {
			...
			if (!initialized || !currentWalletInfo || !currentWalletInfo.selectedWallet) return;
	...
	onEthInputChange(e) {
		...
		const newInputValue = e && e.target.validity.valid ? e.target.value : ethInputValue;
	...
	onSynthInputChange(e) {
		...
		const newInputValue = e && e.target.validity.valid ? e.target.value : synthInputValue;
			...
			<Popup isVisible={isVisible} closePopup={this.closePopup}>
				<div className={styles.depotPopup}>
					...
					<p className={styles.popupDescription}>
						Here you can purchase sUSD through our Depot, which is a platform available through
						Mintr and Swappr that allows sUSD buyers to buy from SNX holders who have minted sUSD.{' '}
					</p>
					{this.renderEthBalance()}
					<div className={styles.inputLabelRow}>
						<span>Amount</span>
						<button className={styles.buyMaxButton} onClick={this.onBuyMaxClick}>
							Buy Max
						</button>
					</div>
					...
					<GweiSelector />
					<button className={styles.buyButton} type="button" onClick={this.buyFromDepot}>
						Buy
					</button>
				</div>
			</Popup>
