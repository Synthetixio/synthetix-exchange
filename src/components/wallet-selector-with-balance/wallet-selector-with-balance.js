...
class WalletSelectorWithBalances extends Component {
	...
	renderTableContent() {
		const { availableWallets, walletPageSize, walletSelectorIndex, onSelectWallet } = this.props;
		const { selectedSynth } = this.state;
		return availableWallets
			.slice(walletSelectorIndex, walletSelectorIndex + walletPageSize)
			.map((wallet, index) => {
				const { collateral = 0, synthetixEscrow = 0, rewardEscrow = 0, eth = 0 } = wallet.balances;
	...
	renderSynthBalanceHeader() {
		...
		return (
			<th className={styles.synthTableHeader}>
				<OutsideClickHandler onOutsideClick={() => this.setState({ synthPickerBoxIsOpen: false })}>
