...
class FeedbackPopup extends Component {
	...
	async submitForm(e) {
		e.preventDefault();
		const { rating, newFeaturesRequest, featuresNotInterestedIn, comments } = this.state;
	...
	renderRatings() {
		...
		return (
			<div className={styles.ratingStarRow} onMouseLeave={() => this.setState({ ratingHover: 0 })}>
				{[1, 2, 3, 4, 5].map(index => {
					return (
						<div
							...
							className={`${styles.ratingStar} ${
								index <= rating || index <= ratingHover ? styles.ratingStarFull : ''
							}`}
							key={index}
						/>
					);
	...
	renderForm() {
		const { newFeaturesRequest, featuresNotInterestedIn, comments } = this.state;
		return (
			<form className={styles.feedbackForm} name="feedback" onSubmit={this.submitForm}>
		...
		return (
			<Popup isVisible={isVisible} closePopup={this.closePopup}>
				<div className={styles.feedbackPopup}>
					<h1>{showThanks ? 'Thanks for your feedback!' : 'Please give us your feedback'}</h1>
...
export default connect(null, mapDispatchToProps)(FeedbackPopup);
