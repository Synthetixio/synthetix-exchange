...
class TradingComp extends Component {
	...
	refreshData = () => {
		this.setState({ loading: true, competitors: null }, async () => {
			...
			competitors = competitors.map(arr =>
				sortBy(arr, elem => -elem.gain + (elem.notes ? Number.MAX_SAFE_INTEGER : 0))
			...
			return (
				<tr key={index}>
					...
					<td>
						{startingBalance == 0
							? '–'
							: numbro(gain || 0).format({ output: 'percent', mantissa: 2 })}
					</td>
					<td>{eligible}</td>
				</tr>
			);
	...
	renderTradingCompTable() {
		...
		return (
			<div className={styles.tableWrapper}>
				<button disabled={loading} onClick={this.refreshData} className={styles.refreshButton}>
					Refresh
				</button>
		...
		return (
			<div style={{ height: '100%' }}>
				<div className={styles.tradingCompLayout}>
					<Container minHeight={'500px'}>{this.renderTradingCompTable()}</Container>
				</div>
			</div>
		);
	}
