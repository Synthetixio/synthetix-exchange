...
class TransactionStatusPopup extends Component {
	...
	renderAmount(amount, synth) {
		return (
			<div className={styles.conversionAmount}>
				<span className={styles.conversionAmountValue}>{numbro(amount).format('0,0.00')}</span>
	...
	renderFees() {
		...
		const feeRate = this.isSwingTrade(fromSynth, toSynth) ? 2 * exchangeFeeRate : exchangeFeeRate;
		...
		return (
			<div>
				...
				<p className={styles.explanationParagraph}>
					To continue, please confirm the trade via {walletType !== 'Metamask' ? 'your ' : ''}
					{walletType}
				</p>
				<Spinner small={true} />
				<div className={styles.transactionPair}>
					{this.renderAmount(fromAmount, fromSynth)}
					<img height={11} src="images/transaction-icon.svg" alt="transaction icon" />
	...
	goToEtherscan() {
		...
		window.open(
			`https://${networkName ? networkName + '.' : ''}etherscan.io/tx/${transactionHash}`
		);
	}
	...
	renderWaitForSuccessContent(success) {
		return (
			<div>
				<img src="images/transaction-in-progress.svg" alt="transaction in progress" />
				<h1>Transaction {!success ? 'in progress' : 'Success'}</h1>
				{!success ? <Spinner small={true} /> : null}
				<button className={styles.buttonVerifyTransaction} onClick={this.goToEtherscan}>
					Verify Transaction
				</button>
			</div>
		);
	...
	renderErrorContent() {
		...
		return (
			<div>
				...
				<button className={styles.buttonVerifyTransaction} onClick={this.closePopup}>
					Back
				</button>
			</div>
		);
...
export default connect(mapStateToProps, mapDispatchToProps)(TransactionStatusPopup);
