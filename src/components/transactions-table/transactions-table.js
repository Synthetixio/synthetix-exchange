...
class Transactions extends Component {
	constructor(props) {
		...
		this.state = {
			...
			transactions: null,
			showAllTrades: !(props.currentWalletInfo && props.currentWalletInfo.selectedWallet),
		};
	...
	async refreshData() {
		...
		const transactions = await getTransactions(currentWalletInfo && currentWalletInfo.networkId);
	...
	componentDidUpdate(prevProps) {
		...
		const selectedWallet =
			this.props.currentWalletInfo && this.props.currentWalletInfo.selectedWallet;
		if (
	...
	toggleShowMyTrades() {
		...
		if (showAllTrades && (!myTransactions || myTransactions.length === 0)) return null;
		this.setState({
	...
	renderTableBody() {
		...
		const { currentWalletInfo } = this.props;
		const networkId =
			currentWalletInfo && currentWalletInfo.networkId ? currentWalletInfo.networkId : 1;
		...
		return transactions.map((transaction, index) => {
			return (
				<tr key={index}>
					...
					<td>
						{numbro(transaction.exchangeFromAmount / transaction.exchangeToAmount).format(
							'0,0.000000'
						)}
					</td>
					<td>
					...
					<td>{format(new Date(transaction.blockTimestamp), 'D/M/YYYY H:mmA ')}</td>
